// ==UserScript==
// @name         Chess.com Maia Chess (Browser-Based)
// @namespace    z-maia-chess
// @version      2.0.0
// @description  Maia chess engine running directly in your browser - no external API needed!
...
// @require      https://unpkg.com/idb@7/build/iife/index-min.js
...
(function() {
  ...
  const log = (...args) => console.log('%c[Maia-Chess]', 'color:#09f', ...args);
  const STORE = {
  ...
  let modelError = null;
  let moveVocab = null; // Array of UCI strings
  ...
  async function loadModel() {
    ...
    try {
      ...
      try {
        ...
        log('Cache error (normal on first run):', e);
      }

      // Fetch model from CDN
      log('Downloading model from CDN...');
  ...
  async function loadMoveVocab(){
    if (moveVocab && moveToIndex) return moveVocab;
    if (vocabLoading) {
      return new Promise((resolve) => {
        const t = setInterval(() => {
          if (moveVocab || vocabError) { clearInterval(t); resolve(moveVocab); }
        }, 100);
      });
    }
    vocabLoading = true;
    try {
      ...
      try {
        const cache = await openModelCache();
        const cached = await cache.get('vocab', 'move-vocab-v1');
        if (cached) {
          moveVocab = JSON.parse(new TextDecoder().decode(cached));
          moveToIndex = new Map(moveVocab.map((m, i) => [m, i]));
          log('Move vocab loaded from cache');
          return moveVocab;
        }
      } catch (e) { /* ignore */ }

      // Fetch from CDN
      log('Downloading move vocabulary...');
      const res = await fetch(MOVE_VOCAB_URL);
      if (!res.ok) throw new Error(`HTTP ${res.status}`);
      const json = await res.json();
      if (!Array.isArray(json)) throw new Error('Invalid vocab JSON');
      moveVocab = json;
      moveToIndex = new Map(moveVocab.map((m, i) => [m, i]));
      ...
      try {
        const cache = await openModelCache();
        await cache.put('vocab', new TextEncoder().encode(JSON.stringify(moveVocab)), 'move-vocab-v1');
      } catch (e) { /* ignore */ }

      log('Move vocabulary loaded');
      return moveVocab;
    } catch (e) {
      vocabError = e;
      log('Failed to load move vocabulary:', e);
      return null;
    } finally {
      vocabLoading = false;
    }
  ...
  function fenToTensor(fen) {
    ...
    // Initialize 1x18x8x8 tensor (1152 elements)
    const tensor = new Float32Array(1152);
    ...
    for (let r = 0; r < 8; r++) {
      for (let f = 0; f < 8; f++) {
        ...
        if (piece) {
          const channel = pieceMap[piece];
          if (channel !== undefined) {
            const index = channel * 64 + r * 8 + f;
            tensor[index] = 1;
          }
        }
      }
    ...
    // Active color channel (12)
    const activeChannel = 12;
    const activeValue = activeColor === 'w' ? 1 : 0;
    for (let r = 0; r < 8; r++) {
      for (let f = 0; f < 8; f++) {
        const index = activeChannel * 64 + r * 8 + f;
        tensor[index] = activeValue;
      }
    }
    
    // Castling rights channels (13-16)
    const castlingChannels = {
      'K': 13, 'Q': 14, 'k': 15, 'q': 16
    };
    for (const [right, channel] of Object.entries(castlingChannels)) {
  ...
  async function getMaiaMove(fen, elo) {
    try {
      // Ensure model and vocabulary are loaded
      const session = await loadModel();
      if (!session) throw new Error('Model not loaded');
      await loadMoveVocab();
      if (!moveToIndex) throw new Error('Move vocabulary not loaded');
      ...
      const inputs = {
        'board_input': new ort.Tensor('float32', tensor, [1, 18, 8, 8]),
        // ONNX expects int64 for ELO embeddings; use BigInt64Array
        'elo_self': new ort.Tensor('int64', BigInt64Array.from([BigInt(eloBucket)]), [1]),
        'elo_oppo': new ort.Tensor('int64', BigInt64Array.from([BigInt(eloBucket)]), [1])
      };
      
      const outputs = await session.run(inputs);
      const moveProbs = outputs.move_probs.data;
      
      // Generate all possible moves and find the one with highest probability
      const chess = new Chess(fen);
      const legalMoves = chess.moves({ verbose: true });
      
      if (legalMoves.length === 0) return null;
      
      // Find move with highest probability among legal moves
      let bestMove = null;
      ...
      return bestMove || legalMoves[0].san;
    } catch (e) {
  ...
  function updateModelStatus(status) {
    modelStatus = status;
    const statusEl = $('#zModelStatus', ui);
    if (statusEl) {
      statusEl.textContent = status;
      statusEl.style.color = status === 'ready' ? '#4caf50' : 
  ...
  function buildUI() {
    ...
    // Initialize model and vocabulary loading
    Promise.all([loadModel(), loadMoveVocab()])
      .then(() => updateModelStatus('ready'))
  ...
  async function fetchMaiaMove(fen, elo) {
    log(`Calculating move for FEN: ${fen} (ELO: ${elo})`);
    
    try {
      const move = await getMaiaMove(fen, elo);
      if (move) {
        log('Maia recommends:', move);
        displayRecommendation(move);
        if (move.length >= 4) {
  ...
  /*************************
   * Chess.com game detection & FEN building
   *************************/
